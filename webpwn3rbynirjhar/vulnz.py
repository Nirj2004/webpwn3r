#!/usr/bin/env python
# WebPwm3r is a Web Application Security Scanner
# By Ebrahim Hegazy - instagram/Nirjhar Biswas
import urllib
import re
from headers import*

#updates:
# 1- Fixed the empty the empty parameters issue => Done.
...
def main_function(url, payloads, check):
        ...
        if vuln == 0:
                ...
                print ga.blue+" [!] " Congrats MotherFucker you've found %i bugs ) " % (vuln) +ga.end

# Here stands the vulnerabilities functions and detections payloads.
def rce_func(url):
    headers_readers(url)
    headers_reader(url)
